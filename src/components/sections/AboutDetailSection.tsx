...
const AboutDetailSection = () => {
  ...
  const nav = (
    <div className="flex font-bold items-center gap-3 md:gap-5 lg:gap-8 text-[10px] md:text-sm lg:text-xl text-center whitespace-nowrap" style={orbitron}>
      ...
      <div className="text-primary cursor-pointer">About</div>
      <div className="text-text hover:text-primary transition-colors cursor-pointer" onClick={() => setCurrentPage('contact')}>Contact</div>
    </div>
  )

  const introText = (
    <div className="text-secondary text-center tracking-widest space-y-2">
      <p>안녕하세요. 저는 <span className="text-primary font-bold">LLM</span> 같은 사람 <span className="font-bold">서희정</span>입니다.</p>
      ...
      <p>앞으로도 새로운 경험을 <span className="text-primary font-bold">두려워하지 않고, 배우고, 도전하며</span> 저만의 길을 만들어나가고 싶습니다.</p>
      <p>감사합니다.</p>
    </div>
  )
  ...
  return (
    <SectionCard nav={nav} clearProfile>
      ...
      <div className="lg:hidden flex flex-col items-center justify-center h-140 md:h-160 px-6 md:px-10 pt-14 pb-14 gap-4 relative z-10">
        <h2 className="text-text font-bold text-3xl md:text-4xl" style={orbitron}>About</h2>
      ...
      <div className="hidden lg:block lg:h-132.5 xl:h-155 2xl:h-175">
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 z-10">
          <h2 className="text-text font-bold text-5xl" style={orbitron}>About</h2>
